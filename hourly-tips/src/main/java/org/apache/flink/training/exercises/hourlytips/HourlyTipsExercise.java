...
public class HourlyTipsExercise {
    ...
    public JobExecutionResult execute() throws Exception {
        ...
        DataStream<Tuple3<Long, Long, Float>> hourlyMax = fares
            ...
            .process(new HourlyTipsProcessWindowFunction());

            // .windowAll(TumblingEventTimeWindows.of(Time.hours(1)))
            // .reduce(new ReduceFunction<Tuple3<Long, Long, Float>>() {
            //     public Tuple3<Long, Long, Float> reduce(
            //         Tuple3<Long, Long, Float> hourlyMaxTipsTupleA, 
            //         Tuple3<Long, Long, Float> hourlyMaxTipsTupleB
            //     ) {
            //         return hourlyMaxTipsTupleA.f2 > hourlyMaxTipsTupleB.f2 
            //             ? hourlyMaxTipsTupleA 
            //             : hourlyMaxTipsTupleB;
            //     }
            // });

        DataStream<Tuple3<Long, Long, Float>> stream = hourlyMax.windowAll(TumblingEventTimeWindows.of(Time.hours(1))).maxBy("2");
        stream.addSink(sink);
    ...
    public static class HourlyTipsProcessWindowFunction 
        extends ProcessWindowFunction<TaxiFare, Tuple3<Long, Long, Float>, Long, TimeWindow> {
            public void process(
                Long key, Context context, Iterable<TaxiFare> in, Collector<Tuple3<Long, Long, Float>> out) throws Exception {
                float sum = 0;
                for (TaxiFare t : in) {
                    sum += t.tip;
                }
                out.collect(new Tuple3<Long, Long, Float>(context.window().getEnd(), key, sum));
            }
    }
}
